...
- Delete one-time alarm: "wecker löschen", "lösche den wecker" / "delete the alarm", "delete alarm" - deletes it if exactly one is currently set; if none are set, or more than one is set, you get a spoken message instead of a guess (name a weekday/date isn't supported yet for narrowing this down - see Status)
